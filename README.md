...
* `tests/build_docker.sh` drives the container build. The test project to build is taken from the current directory, so run it from inside the test folder you want to build (`tests/xxxx`).
```
...
$ ../build_docker.sh pico      # build only Pico    -> build/pico_fatfs_xxxx.uf2
$ ../build_docker.sh pico2     # build only Pico 2  -> build2/pico_fatfs_xxxx.uf2
```
* Outputs: `build/pico_fatfs_xxxx.uf2` (Pico), `build2/pico_fatfs_xxxx.uf2` (Pico 2)
...
| <a href="doc/benchmark/09 - Kioxia Exceria G2 256GB/">9</a> | <img src="doc/microsd/kioxia_exceria_g2_256g.jpg" width="60" /> | Kioxia  | Exceria G2 | 256GB | LMEX2L256GG2 | microSDXC UHS-I U3 V30 | exFAT |
